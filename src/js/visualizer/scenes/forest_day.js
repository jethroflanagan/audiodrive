...
return function () {
VISU.scenes.ForestDay = function () {

	VISU.Scene.call( this );

	//BACKGROUND LAYER
	var background = new VISU.Layer( this );
	background.loadImage( "trees", "/forest/day/background.jpg" );
	...
	for (var i = 20; i >= 0; i--) {
		...
		p.spawn = function( ctx ) {
			...
			this.x = this.targetX;
			this.y = this.targetY;
			this.scale = (1.3 + Math.random()) * 0.2;
			this.opacity = (0.05 + Math.random()) * 0.7;
...
VISU.scenes.ForestDay.prototype = Object.create( VISU.Scene.prototype );
VISU.scenes.ForestDay.prototype.constructor = VISU.scenes.ForestDay;
}